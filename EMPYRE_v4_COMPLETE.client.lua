...
function Runtime.UpdateCrosshair(point)
    ...
    if Runtime.CrosshairDot then
        Runtime.CrosshairDot.Visible = visible
    end
...
local BrandSub = text(Brand, "v4.0", 9, COLORS.Muted, Enum.Font.GothamBold)
...
local InfoTitle = text(InfoCard, "EMPYRE v4.0", 13, COLORS.White, Enum.Font.GothamBold)
...
local function createESP(entity)
    ...
    for index = 1, #SKELETON_CONNECTIONS do
        SkeletonLines[index] = makeLine(Holder)
        SkeletonLines[index].AnchorPoint = Vector2.new(0, 0.5)
    end
...
Workspace.DescendantAdded:Connect(function(instance)
    ...
    if instance:IsA("Model") then
        ...
        model = findHumanoidModel(instance)
    end

    if model and not pendingBotRegistration[model] then
...
function Runtime.UpdateSpinbot(deltaTime)
    ...
    local pause = not CONFIG.SpinbotEnabled
        or not character
        or not humanoid
    ...
    if Runtime.SpinCharacter ~= character
        ...
        or not Runtime.SpinMotor.Parent then
...
local function temporarilyKeepTarget(entity, now)
    if not CONFIG.StickyTarget
        or not entity
        or now - targetLastValidAt > CONFIG.LockGrace then

        return false
    end

    local candidate = targetCandidate(entity, false, true)
...
RunService:BindToRenderStep(
    ...
    function(deltaTime)
        ...
        if PerformanceHud.Visible then
        ...
        FOVCircle.Visible =
            CONFIG.ShowFOV and CONFIG.AimEnabled
        FOVStroke.Color = colour

        if now >= nextVisualListRefresh then
            nextVisualListRefresh = now + 0.35
...
function Runtime.GetCurrentFOV()
    ...
    if currentTarget then
        result *= CONFIG.DynamicFOVLockedScale
    end
